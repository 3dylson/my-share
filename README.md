...
- **Privacy-First Ads**: AdMob integration fortified by the Google User Messaging Platform (UMP) SDK to respect GDPR/CCPA. Ads are strictly limited to non-financial surfaces (MoreTab) and disabled entirely for Premium users.
...
* **Security**: Keystore secrets and localized `admobAppId` configurations are cleanly abstracted into `local.properties` environments.

## 🚀 Getting Started

1. Clone the repository.
2. In the root directory, create a `local.properties` file with your specific environment keys if creating a release build:
   ```properties
